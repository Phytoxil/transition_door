...
class GroupClass(QGroupBox):
    ...
    def init(self):
        ...
        ledNosePokeLeft = QVBoxLayout()
        ledNosePokeRight = QVBoxLayout()
        ...
        # self.fields.addWidget(ledNosePokeLeft, 3, 2, 1, 1)
    ...
    def connect(self):
        ...
        if self.serial is None:
            ...
            self.fed = Fed3Manager(comPort=self.typeBox.currentText())
            time.sleep(0.05)
    ...
    def sendData(self):
        if self.fed:
            if self.title.text() != "":
                ...
                self.fed.controlLight(self.title.text())
                # answer = self.readData()
                # if (self.title.text().encode() == "scan"):
                print("Send -> " + self.title.text())
                # else:
