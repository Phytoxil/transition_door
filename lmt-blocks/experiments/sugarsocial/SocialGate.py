...
from blocks.autogate.Parameters import NB_VALUE_TO_COMPUTE_MEAN_WEIGHT,\
    MIN_BALANCE_WEIGHT_GRAPH, MAX_BALANCE_WEIGHT_GRAPH,\
    DEFAULT_TORQUE_AND_SPEED_LIMIT_MOUSE
from blocks.autogate.Gate import GateOrder, Gate
...
class WDoor(QtWidgets.QWidget):
    ...
    def __init__(self, name="noName", door=None , *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.setStyleSheet(StyleSheet)
...
class WGate(QtWidgets.QWidget):
    ...
    def __init__(self, gate , name="Gate 1", *args, **kwargs):
        ...
        self.wDoorA = WDoor(name ="Door A", door = gate.doorA )
        horizontalLayout.addWidget( self.wDoorA ) 
        
        self.wDoorB = WDoor(name= "Door B" , door = gate.doorB )
        ...
        self.stopped = False
        thread = threading.Thread( target=self.monitorGateGUI )
...
if __name__ == '__main__':
    ...
    quit()
    '''
    gate.setRFIDControlEnabled ( False ) # will / will not perform ID check    
    #gate.addForbiddenRFID("001039048351")
        
    #gate.setSpeedAndTorqueLimits(150, 150)
    gate.setSpeedAndTorqueLimits(DEFAULT_TORQUE_AND_SPEED_LIMIT_MOUSE, DEFAULT_TORQUE_AND_SPEED_LIMIT_MOUSE)
